...
export function gradientToCss(gradient, fallback = "#ffffff") {
  if (!gradient || !Array.isArray(gradient.colorStops) || !gradient.colorStops.length) {
    return fallback;
  }
  ...
  const stops = [...gradient.colorStops]
    ...
    .map((stop) => `${stop.color} ${Math.round(clamp01(stop.offset) * 100)}%`)
    .join(", ");

  return `linear-gradient(${angle}deg, ${stops})`;
}

export function createCanvasGradient(ctx, width, height, gradient) {
  if (!ctx || !gradient) return null;
  const angle = getGradientAngleDegrees(gradient, DEFAULT_ANGLE_DEG);
  ...
  const colorStops = Array.isArray(gradient.colorStops) ? gradient.colorStops : [];
